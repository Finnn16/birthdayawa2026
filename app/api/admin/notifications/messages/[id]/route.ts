...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    if (updateError) {
      return NextResponse.json({ error: "Gagal update message.", details: updateError.message }, { status: 500 });
    }

    return NextResponse.json({ message: data });
  } catch (error) {
    return NextResponse.json(
      { error: "Gagal update message.", details: error instanceof Error ? error.message : "Unknown error" },
      { status: 500 },
    );
  }
}
...
export async function DELETE(_req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    if (deleteError) {
      return NextResponse.json({ error: "Gagal menghapus message.", details: deleteError.message }, { status: 500 });
    }

    return NextResponse.json({ deleted: true });
  } catch (error) {
    return NextResponse.json(
      { error: "Gagal menghapus message.", details: error instanceof Error ? error.message : "Unknown error" },
      { status: 500 },
    );
  }
}
